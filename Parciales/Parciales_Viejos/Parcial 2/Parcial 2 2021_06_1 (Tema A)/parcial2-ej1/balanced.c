...
static char *take_delimeters(char *expression, size_t *count)
{
    ...
    size_t n_delimeters = count_delimeters(expression);
    char *only_delimeters = malloc(sizeof(char) * (unsigned int)n_delimeters);
    assert(only_delimeters != NULL);
    while (expression[i] != '\0')
    {
